...
area_calculations_res <- function(df_in, EOMbackgroundsubstraction) {

  if(EOMbackgroundsubstraction=="si"){
    ...
    # Remove individual oxidized/unoxidized columns
    df_out <- df_out %>%
      select(-Sample_Oxidized, -Sample_Unoxidized)
    ...
    # Calculation of Standard Deviation
    df_out$SD <- sqrt(df_out$VarianceSample)
    # Filter out rows with missing Res or EOM values
    #df_out <- df_out[complete.cases(df_out[c("Res", "EOM")]), ]
